...
Interactive commands:
  ...
  /payload     -> show last payload sent (raw)
  Ctrl-D (EOF) -> send payload / resend last payload
EOF
  exit 1
}
...
while true; do
  ...
  echo "  /show        -> show conversation pretty"
  ...
  echo "  Ctrl-D (EOF) -> send payload / resend last payload"
  echo
  ...
  case "$user_text_trimmed" in
    "/show")
      echo
      echo "-> Conversation (pretty)"
      len=$(jq length "$CONVO_FILE")
      if (( len == 0 )); then
        echo "[conversation empty]"
      else
        jq -c '.[]' "$CONVO_FILE" | while read -r msg; do
